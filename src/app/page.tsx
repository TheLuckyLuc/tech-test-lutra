...
export default function Home() {
  ...
  const appointmentsResponse = api.appointment.getAppointments.useQuery();
          ...
          <div className='grid grid-cols-1 gap-4 pb-1 sm:grid-cols-2 lg:grid-cols-3'>
            {patientsResponse.data ? (
              patientsResponse.data.map(patient => (
                <div key={patient.id} className='group relative rounded border border-gray-200 bg-white p-2'>
                  <div className='h-20'>
                    <div className='relative h-full overflow-hidden rounded bg-gray-50'>
                      <svg className='absolute inset-0 h-full w-full stroke-blue-100' fill='none'>
                  ...
                  <h3 className='mt-2 font-semibold'>
                    <Link href={`/patient/${patient.id}`} className='focus:outline-none group-hover:text-blue-600'>
                      <span className='absolute inset-0' aria-hidden={true} />
                      {patient.firstName} {patient.lastName}
                    </Link>
                  </h3>
                  <p className='text-gray-600'>{patient.isActive ? 'Active' : 'Inactive'}</p>
                </div>
              ))
            ) : (
          ...
          <div className='grid grid-cols-1 gap-4 pb-1 sm:grid-cols-2 lg:grid-cols-3'>
            {appointmentsResponse.data ? (
              appointmentsResponse.data.map(appointment => (
                <div key={appointment.id} className='group relative rounded border border-gray-200 bg-white p-2'>
